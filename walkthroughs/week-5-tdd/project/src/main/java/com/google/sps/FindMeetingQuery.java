...
public final class FindMeetingQuery {
  ...
  public Collection<TimeRange> query(Collection<Event> events, MeetingRequest request) {
    ...
    ImmutableList<TimeRange> optionalUnavailableTimes =
    ...
    // Find all available time ranges for the requested meeting based on the attendees' unavailable
    // times
    ...
    ImmutableList<TimeRange> optionalAvailableTimes =
        getAvailableTimeRanges(optionalUnavailableTimes, request);
    ...
    } else if (optionalAvailableTimes.isEmpty()
        || optionalAvailableTimes.contains(TimeRange.WHOLE_DAY)) {
      // No need to check for overlapping times if optional available times do not provide time
      // constraints
      return availableTimes;
    }

    // Find and return overlapping available time ranges if there are any
  ...
  public final ImmutableList<TimeRange> getAvailableTimeRanges(
      ImmutableList<TimeRange> unavailableTimes, MeetingRequest request) {
  ...
  private static TimeRange findAvailableTimeRange(
      TimeRange currentUnavailableTime, int nextAvailableStart, MeetingRequest request) {
    TimeRange timeRangeAvailable;
    ...
    if (timeDifference >= request.getDuration()) {
      timeRangeAvailable =
          TimeRange.fromStartEnd(nextAvailableStart, currentUnavailableTime.start(), false);
    } else {
  ...
  private static ImmutableList<TimeRange> getOverlappingTimeRanges(
      ImmutableList<TimeRange> requiredTimes,
      ImmutableList<TimeRange> optionalTimes,
    ...
    while (optionalTimeIndex < optionalTimes.size()) {
      ...
      if (currentRequiredTime.overlaps(currentOptionalTime)) {
        ...
        int start =
            Collections.max(
                ImmutableList.of(currentRequiredTime.start(), currentOptionalTime.start()));
        int end =
            Collections.min(ImmutableList.of(currentRequiredTime.end(), currentOptionalTime.end()));

        // Add time range only if it can fit the requested meeting duration
        if (end - start >= request.getDuration()) {
          overlappingAvailableTimes.add(
              TimeRange.fromStartEnd(start, end, end == TimeRange.END_OF_DAY));
        }
        requiredTimeIndex += 1;
      }
      optionalTimeIndex += 1;
